...
## Kotlin은 왜 래퍼 클래스가 필요 없을까?
Kotlin은 자바와 달리 **기본형(primitive type)** 과 **객체형(wrapper type)** 을 개발자가 구분하지 않아도 되도록 설계되어 있습니다.  
내부적으로는 JVM 위에서 동작하므로 Int, Double 등은 상황에 따라 자동으로 int, Integer로 변환됩니다.
...
### 🔍 조금 더 자세히 설명하자면:
- Kotlin은 기본적으로 Int 하나만 제공. 개발자는 int와 Integer를 구분할 필요 없이 Int 또는 Int?만 사용하면 됨.
...
- 즉, nullable 여부에 따라 내부적으로 자바의 int인지 Integer인지 결정됨.
